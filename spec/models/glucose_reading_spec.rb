...
RSpec.describe GlucoseReading do
  let(:user) do
    User.create!(first_name: "John", last_name: "Connor")
  end
  
  it 'persists a glucose reading' do
    glucose_reading = GlucoseReading.new({ bg_value: 2, time_stamp: "11:00", user: user})
  ...
  context "bg_value" do
    it 'may be set as a string' do
      glucose_reading = GlucoseReading.create!({ bg_value: "2", time_stamp: "11:00", user: user})

      expect(glucose_reading.bg_value).to eq(2)
    end

    it "does not truncate a decimal" do
      glucose_reading = GlucoseReading.create!({ bg_value: "9.78", time_stamp: "11:00", user: user})
